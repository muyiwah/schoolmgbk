...
class _TimeTableCreatorScreenState extends State<TimeTableCreatorScreen> {
  ...
  void _initializeData() {
    setState(() {
      // Initialize each day with generated periods
  ...
  List<String> _generatePeriods() {
    ...
    int periodCount = 0;
    bool isBreakAdded = false;

    for (int i = 0; i < _numberOfPeriods; i++) {
      // Add short break after specified number of periods
      if (_breakAfterPeriods > 0 &&
          periodCount > 0 &&
          periodCount % _breakAfterPeriods == 0 &&
          !isBreakAdded) {
        final breakEnd = _addMinutes(currentTime, _breakDuration);
        periods.add(
          '${_formatTime(currentTime)}-${_formatTime(breakEnd)} (Break)',
        );
        currentTime = breakEnd;
        isBreakAdded = true;
        continue; // Skip incrementing period count for breaks
      }
      ...
      if (_includeLunchBreak &&
        ...
        periods.add(
          '${_formatTime(currentTime)}-${_formatTime(lunchEnd)} (Lunch)',
        );
        currentTime = lunchEnd;
        isBreakAdded = true;
        continue; // Skip incrementing period count for lunch
      }

      // Add regular period
      final periodEnd = _addMinutes(currentTime, _periodDuration);
      periods.add('${_formatTime(currentTime)}-${_formatTime(periodEnd)}');
  ...
  void _updateAllPeriods() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Periods updated for all days')));
  }
  ...
  void _addDay(String day) {
    ...
    setState(() {
      // Add new day with generated periods
      _dayPeriods[day] = _generatePeriods();
      _resetTimetable();
    });
  }
  ...
  void _removeDay(String day) {
    ...
    ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('You must have at least one day')));
  ...
  void _removePeriodFromDay(String day, int periodIndex) {
    if (_dayPeriods[day]!.length <= 1) {
      // Fluttertoast.showToast(msg: "You must have at least one period");
       ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('You must have at least one period')));
      return;
    }
    setState(() {
                    ...
                    return InkWell(
                      onTap:
                          () =>
                              isSpecial
                                  ? null
                                  : _editCell(periodIndex, dayIndex),
                      child: SizedBox(
                        width: 180,
